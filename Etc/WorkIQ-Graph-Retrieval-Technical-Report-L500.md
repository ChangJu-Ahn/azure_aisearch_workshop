...
> ℹ️ **공개 배포 고지 — 식별자 익명화**: 이 문서는 실제 라이브 테넌트에서 수행한 실측 기록입니다. 공개 저장소 게시를 위해 **테넌트/구독/앱/사용자 고유 식별자는 `<PLACEHOLDER>` 형태로 마스킹**했습니다(예: `<TENANT>`, `<TENANT_M365_ID>`, `<SUBSCRIPTION_ID>`, `<ENTRA_APP_ID>`, `<ADMIN_UPN>`, `<SEARCH_SERVICE>`, `<request-id>` 등). 단, **Microsoft 1st-party 공개 앱 ID**(예: Work IQ `fdcc1f02-…`, Work IQ Tools `ea9ffc3e-…`, WorkIQ MCP 클라이언트 `ba081686-…`, Graph CLI `14d82eec-…`, Azure CLI `04b07795-…`)는 공개 상수이므로 원문 유지했습니다. 방법론·파라미터·응답 구조 등 기술 내용은 원본 그대로입니다.

> 대상 독자: L500 D+ 개발자/컨설턴트
> 테스트 테넌트: `<TENANT>.onmicrosoft.com` (tenantId `<TENANT_M365_ID>`)
> 계정: `<ADMIN_UPN>` (MOD Administrator, objectId `<ADMIN_OBJECT_ID>`)
...
>
> **검증 상태**: ✅ Graph `/me` 200 · ✅ Retrieval `/copilot/retrieval` **200(원문 청크)** & 403(스코프 강제) · ✅ WorkIQ `ask` **200(합성 답변+인용, CLI 경유)** · ✅ **WorkIQ MCP 완전 raw JSON-RPC 왕복**(initialize 200 · tools/list 11툴 · search_paths 성공 · fetch=소비 게이트 · ask=substrate Forbidden) · ✅ WorkIQ MCP 401+PRM · ✅ SP 프로비저닝 · ✅ 라이선스 실측 · ✅ **공식 문서 교차검증 완료(§10)**
...
| 과금 | 없음(대부분 무료) | **Copilot 라이선스 또는 PAYG** | **Copilot Credits**(소비) | **Copilot Credits**(GA 시 라이선스 독립) |
...
- **⭐ 실 테넌트 E2E 실증(§11.10):** M365 Copilot 테넌트(`<TENANT_M365_ID>`, 플래그 `Registered`)에서 — `remoteSharePoint` KS는 **HTTP 200 + 실제 SharePoint 문서**(Fabrikam/Contoso) 반환 ✅; `workIQ` KS는 **FIC→OBO→A2A 인증 체인 완전 통과**(work-iq 헤더 토큰 `aud`=Entra 앱 필수, 이중 헤더·RAW 토큰) 후 **유일한 잔여 게이트 = Copilot Credits(AI credits)**. **라이선스 경계 확정**: 대화형 `ask`는 **Copilot 좌석에 포함**(§5.4 200), **Foundry IQ의 `workIQ`(Work IQ API/A2A)는 좌석과 별개로 Copilot Credits 소비**(2026-06-16 GA, 관리자 billing 활성화 필요).
...
| az CLI 컨텍스트 | 구독 `ME-<TENANT>-...` (`<SUBSCRIPTION_ID>`)로 전환, Graph 토큰 캐시 유효 |
| 검증 경로 | ① Graph 실호출(200) ② Retrieval 실호출(403 스코프 강제 + **200 원문 청크**) ③ WorkIQ MCP 무인증 프로브(401+PRM) ④ **WorkIQ MCP 완전 raw JSON-RPC 왕복**(initialize/tools_list/tools_call) ⑤ SP 프로비저닝(Graph write) ⑥ 라이선스 실측 |
...
| **3** | **위임 전용 + Copilot 과금 + 특수 스코프** | app-only 광범위 지원, 무과금 | **app-only 미지원**, `Files.Read.All`+`Sites.Read.All` 필수, Copilot 라이선스/PAYG |
...
- **M365 Copilot 애드온 라이선스** → SharePoint + OneDrive + 커넥터 전체(호출 무료).
- **Pay-as-you-go (preview)** → 비라이선스 사용자를 위한 종량 과금(**호출당 약 $0.10**, Azure 미터 `m365copilotretrievalapi`). **테넌트 레벨 소스만**(SharePoint/커넥터; **OneDrive 개인 소스 제외**). 전제: 테넌트에 **Copilot 라이선스 ≥1석** + 관리자 PAYG 활성화.
- **인증**: 반드시 **delegated**(사용자 컨텍스트) + `Files.Read.All`+`Sites.Read.All`(커넥터는 `ExternalItem.Read.All`). **app-only 미지원**(문서 명시).

> ✅ **이 테넌트 실측(§7.1)**: `admin@<TENANT>...`는 `Microsoft_365_Copilot` 보유 → **애드온 라이선스 전제 충족**. 따라서 §3.4의 403은 **라이선스가 아니라 순수 토큰 스코프(Files/Sites.Read.All) 문제**로 확정됨.
> ⚠️ **문서 대조 주의**: 실측 플랜 `M365_COPILOT_INTELLIGENT_SEARCH`는 licensing-service-plan-reference에 실재하는 Copilot 서비스 플랜이나, **Retrieval 공식 문서는 "Copilot 애드온 라이선스"로만 표기**하고 이 플랜명을 Retrieval 전제로 명시하지 않음 → "이 플랜이 Retrieval을 grounding한다"는 **추론**으로 취급(§10 검증).
...
```
> ✅ **L500 인사이트**: WorkIQ MCP는 **MCP Authorization 스펙(RFC 9728 OAuth Protected Resource Metadata)** 을 정식 구현. 리소스 앱 `fdcc1f02`(=Work IQ), **단일 스코프** `WorkIQAgent.Ask`, org-only v2.0 AS. Graph의 리소스별 다중 스코프 모델과 근본적으로 다른 **"단일 동의 + 서버측 정책"** 거버넌스.
...
```
**토큰 클레임(Output, 실측·마스킹)**: `aud=fdcc1f02-fc51-4226-8753-f668596af7f7`, `scp=WorkIQAgent.Ask`, `upn=<ADMIN_UPN>` → **PRM이 요구한 단일 스코프 그대로**.
...
> 2. **소비 과금(entitlement) 층**: 데이터/소비 툴(`fetch` 등)은 **`"billing policy and AI credit entitlement"`** 로 거부 → **Copilot Credits 소비 게이트의 실측 증거**. 스코프·라이선스와 **직교**(사용자는 Copilot 라이선스 보유·§7.1, 토큰도 유효한데 거부됨).
> 3. **다운스트림 substrate(OBO) 층**: `ask`는 실행까지 도달 후 서버가 BizChat substrate로 OBO하는 지점에서 **`Forbidden`**.
>
> **discovery 툴(`search_paths`/`get_schema`)은 무과금** → 통과. **소비 툴(`ask`/`fetch`/엔티티)은 과금·entitlement 필요** → 게이트. 이는 "Work IQ = 라이선스가 아니라 **Copilot Credits 소비 모델**"이라는 §7 논지를 **에러 문자열 수준에서 확증**한다.
>
> ⚠️ **CLI `ask`(§5.4, 200) vs raw `ask`(Forbidden) 불일치 해석**: 동일 public client(`ba081686`)·동일 계정인데 결과가 다름. 원인은 스코프/라이선스가 아니라 **호출 컨텍스트에 결부된 billing policy/AI credit entitlement 상태**(가용 크레딧 소진 또는 CLI 셋업이 부여하는 정책 유무)로 판단됨. → **호스티드 MCP의 소비 툴은 테넌트에 usage-based billing plan(Copilot Studio)이 결부돼 있어야 안정적으로 200**.

### 4.5 라이선스

Work IQ와 동일(§5.5). 핵심: 리소스 앱 `fdcc1f02` **동의(`WorkIQAgent.Ask`)** + **SP 프로비저닝** + **usage-based billing plan(Copilot Credits)**.
...
| **3** | **소비 기반 과금 + 라이선스 독립** | 무과금 | **Copilot Credits** 소비, **GA 시 Copilot 라이선스와 독립** |
...
- **GA**: **Copilot Credits(소비 과금)**, Copilot 라이선스와 **독립**. 전제: **usage-based billing plan**(Copilot Studio + Azure 구독 + 리소스 그룹) + **사용자 할당**(적용 15–30분).
- **현재/preview**: M365 Copilot 라이선스 + Work IQ Tools(`ea9ffc3e`, 라이브 displayName `Agent Tools`)/Work IQ 리소스(`fdcc1f02`) **SP 프로비저닝** 필요.
...
| 과금 | 무료 | Copilot/PAYG | Copilot Credits | Copilot Credits |

---

## 7. 라이선스 매트릭스 (요약)

| 기술 | 필요 라이선스/과금 | 인증 전제 | 추가 프로비저닝 |
|---|---|---|---|
| **Graph API** | 없음(Entra 앱 등록만; 일부 프리미엄 데이터는 해당 워크로드 라이선스) | delegated 또는 app-only | 앱 등록 + Graph 권한 동의 |
| **Retrieval API** | **M365 Copilot 애드온** 또는 **PAYG(preview)** | **delegated 전용** + `Files.Read.All`+`Sites.Read.All`(+`ExternalItem.Read.All`) | Copilot 배포 |
| **Work IQ MCP** | **Copilot Credits**(usage plan) | OAuth 단일 스코프 `WorkIQAgent.Ask` | **SP(fdcc1f02+ea9ffc3e) 프로비저닝** + billing plan + 사용자 할당 |
| **WorkIQ** | **Copilot Credits**(GA 시 라이선스 독립) | WorkIQ 리소스 토큰 | 상동 + Copilot Studio usage plan |

### 7.1 이 테넌트 라이선스 실측 (Graph `GET /me/licenseDetails`, `/subscribedSkus`)

| 항목 | 실측 결과 |
|---|---|
| 사용자 할당 라이선스 | ✅ **`Microsoft_365_Copilot`** (+ `Microsoft_365_E5_(no_Teams)`, `M365_TEAMS_PREMIUM`) |
| Copilot 서비스 플랜 | `M365_COPILOT_INTELLIGENT_SEARCH`, `M365_COPILOT_SHAREPOINT`, `M365_COPILOT_BUSINESS_CHAT`, `GRAPH_CONNECTORS_COPILOT`, `M365_COPILOT_CONNECTORS`, `M365_COPILOT_APPS`, `M365_COPILOT_TEAMS`, `COPILOT_STUDIO_IN_COPILOT_FOR_M365` |
| 테넌트 SKU | `Microsoft_365_Copilot` — **25석 enabled / 25 consumed** |
| **Work IQ 전용 서비스 플랜** | ❌ **없음** → Work IQ는 라이선스 플랜이 아니라 **Copilot Credits(소비)** 임을 실증 |

**해석 (L500)**
- **Retrieval**: 근거 플랜(`M365_COPILOT_INTELLIGENT_SEARCH`+`M365_COPILOT_SHAREPOINT`) 보유 → **라이선스 충족**. 남은 블로커는 오직 **토큰 스코프**(Files/Sites.Read.All). 즉 §3.4의 403은 라이선스가 아님.
- **WorkIQ `ask`**: bizchat 백엔드 플랜(`M365_COPILOT_BUSINESS_CHAT`) 보유 → Copilot 측 전제 충족. SP 프로비저닝도 완료. 남은 변수는 GA 소비 과금(Copilot Credits/usage plan) 할당 여부뿐.
- **결론**: 이 테넌트에서 3개 기술의 **라이선스 전제는 사실상 충족**되어 있으며, 실제 성공 호출을 막는 건 **(a) delegated 토큰 스코프, (b) 인터랙티브 로그인** 두 가지 운영 요소.
...
2. **EULA** — `workiq accept-eula` → 수락 완료(ref: github.com/microsoft/work-iq-mcp).
...
4. **근본원인 진단 & 조치(FIX)** — Graph write로 SP 2개 생성/확인:
   - `az ad sp create --id fdcc1f02-...` → **Work IQ**(objId `<SP_OBJ_WORKIQ>`)
   - `az ad sp create --id ea9ffc3e-...` → **Agent Tools**(objId `<SP_OBJ_AGENTTOOLS>`)
   - ✅ 실측 확인(테이블):
     ```
     AppId                                 Name         ObjId
     fdcc1f02-fc51-4226-8753-f668596af7f7  Work IQ      <SP_OBJ_WORKIQ>
     ea9ffc3e-8a23-4a7d-836d-234d7c7565c1  Agent Tools  <SP_OBJ_AGENTTOOLS>
     ```
...
10. **라이선스 실측** — `GET /me/licenseDetails` + `/subscribedSkus` → 사용자 **`Microsoft_365_Copilot` 보유**(intelligent search/sharepoint/business chat 플랜), 테넌트 25/25석, **Work IQ 전용 플랜 없음** → 라이선스 전제 충족, 403은 스코프 문제 확정. [§7.1]
...
13. **WorkIQ MCP 완전 raw 왕복 (CLI 없이)** ✅ — device-code(client `ba081686`, scope `fdcc1f02/WorkIQAgent.Ask`)로 위임 토큰 획득(`aud=fdcc1f02`,`scp=WorkIQAgent.Ask` 실측) → 손수 구성한 JSON-RPC로 `POST /mcp` 호출: `initialize` **200**(serverInfo `WorkIQ.MCP.Server v1.0.165.0`) → `notifications/initialized` **202** → `tools/list` **성공(11개 툴, raw명 확정)** → `tools/call`: `search_paths` **성공**, `fetch`는 **`"billing policy and AI credit entitlement"` 거부**(Copilot Credits 소비 게이트 실측), `ask`는 실행 후 substrate **`Forbidden`**. 인증/소비/OBO **3계층 분리**를 에러 문자열로 확증. [§4.4-b]

**결론**: **3개 기술 전부 라이브 실측 완료** — Graph 200, **Retrieval 200(원문 청크+relevanceScore) & 403(스코프 강제)**, **WorkIQ `ask` 200(합성 답변+인용, CLI 경유) + WorkIQ MCP 완전 raw JSON-RPC 왕복(initialize/tools/list/search_paths 성공, fetch=소비 게이트, ask=substrate Forbidden)**, WorkIQ MCP 401/PRM, SP 프로비저닝, 라이선스까지 모두 실제 호출로 검증. `ask`는 **CLI(§5.4)와 raw(§4.4-b)** 두 경로 모두 실측.
...
workiq --account <ADMIN_UPN> ask \
  -q "List my 3 most recent emails and briefly summarize what each is about." -v
```
- SP(§8-4)·라이선스(§7.1) 충족으로 브라우저 로그인 후 정상 동작. billing plan 미할당 시 `403(스코프 에러 아님)`가 유일 변수였으나, 이 테넌트에선 **정상 200**.
...
|---|---|---|
| Retrieval | `.../api/ai-services/retrieval/overview`, `.../copilotroot-retrieval`, `.../resources/retrievalextract`, `.../concepts/retrieval-api-paygo`(PAYG) | 2026-08-07 |
...
| 라이선스 | `entra/identity/users/licensing-service-plan-reference` | 2026-08-14 |
...
| **L** | **라이선스 = "Copilot 라이선스 필요"** | 🔴→🟠 | **2-tier로 갱신**: 애드온 라이선스=무료 / **비라이선스=PAYG preview 호출당 $0.10**(미터 `m365copilotretrievalapi`, **SharePoint+커넥터만·OneDrive 제외**, 테넌트 Copilot≥1석 필요). §3.5·§7 반영 완료 |
| — | `M365_COPILOT_INTELLIGENT_SEARCH`가 Retrieval을 grounding | 🟡 | 해당 플랜은 실재(licensing-ref)하나 Retrieval 문서가 명시 안 함 → **추론**으로 완화(§3.5) |
...
| 과금/거버넌스 | Copilot Credits + **관리자 spending policy 별도 필요**; Rego/OPA 정책 엔진 | ✅ | 문서 일치 → **§4.4-b 라이브 소비 게이트(“billing policy and AI credit entitlement”)와 정합** |
...
| 5 | 과금=**Copilot Credits**, GA 시 라이선스 독립 | ✅ | 뉘앙스: 커스텀/서드파티 에이전트는 **Copilot 라이선스 보유자도 과금**, 빌트인 Copilot 경험은 추가 과금 X, 관리자가 Admin Center에서 billing 활성화 |
| 6 | 과금 구조 = **하이브리드**(Tools=고정, Chat/Context=변동) | ✅ | devblog 일치(신규 확인) |
| 7 | **Work IQ 전용 서비스 플랜 없음** | ✅ | licensing-ref 대조 + §7.1 테넌트 실측 일치 |

### 10.5 결론 & 교정 요약

- **전체 판정**: 보고서의 라이브 실측은 **최신 공식 문서와 광범위하게 일치**(Retrieval 7/8, Work IQ MCP 핵심 6/6, 플랫폼 7/7 CONFIRMED). 라이브에서 잡은 값 중 문서에 없던 2건(`/mcp` PRM 경로·`/organizations/v2.0` AS)은 **라이브 증거가 문서보다 최신·authoritative**.
- **적용한 교정(inline 반영 완료)**:
  1. 🟠 `ea9ffc3e` → **"Work IQ Tools"**(공식명) + 라이브 displayName `Agent Tools` 병기 (부록 A, §8).
  2. 🟠 `fdcc1f02` = **리소스/오디언스 앱** 역할 명확화 (§10.3, §8).
  3. 🔴 Retrieval 라이선스 → **2-tier(애드온 무료 / PAYG $0.10·SharePoint+커넥터·OneDrive 제외·Copilot≥1석)** (§3.5, §7).
  4. 🟠 툴 개수 = **docs 10 / live 11(+fetch_blob)** (§0, §4.2).
  5. 🟡 `M365_COPILOT_INTELLIGENT_SEARCH`-grounds-Retrieval는 **추론**으로 완화 (§3.5).
- **신규 사실 추가**: Copilot Credits **고정(Tools)+변동(Chat/Context)** 구조; Retrieval `$batch`(20)·dataSource 1종/호출·파일 크기·시맨틱 확장자·`dataSourceConfiguration`; enable 스크립트의 **~10 sub-server SP** 프로비저닝.
- **미해결/후속**: PAYG 실과금 경로·`m365copilotretrievalapi` 미터는 이 테넌트에서 미검증(관리자 PAYG 활성화 필요). Work IQ MCP entity 도구의 소비 게이트 해제(Copilot Credits spending policy)도 미검증 — 두 가지가 다음 라이브 검증 후보.
...
### 11.8 접근 게이트 & 라이선스 (Foundry IQ 관점)

| KS | 필요 게이트 / 라이선스 |
|---|---|
| `workIQ` | ① 피처플래그 `EnableFoundryIQWithWorkIQ` 등록(**라이브 Pending 실측**) → ② `az provider register -n Microsoft.Search` → ③ **Entra 관리자**가 `aka.ms/foundry-iq-work-iq-admin-consent-form` 제출 → ④ **Microsoft 수동 승인**. + **동일 테넌트**(Search·WorkIQ·user) + **M365 Copilot 라이선스/user** + Entra 앱(FIC) + ⑤ **Copilot Credits(AI credits) consumption billing 활성화** — **§11.10에서 최종 게이트로 실측**(A2A는 좌석과 별개로 크레딧 소비, 2026-06-16 GA) |
| `remoteSharePoint` | **§3.5와 동일**: M365 + **Copilot 라이선스(또는 Retrieval PAYG)** |
| 공통 | Azure AI Search 서비스 비용 별도(컨트롤플레인) |
...
| KB 합성 모델 | `gpt-5.4-mini` @ `<AOAI_RESOURCE>-…openai.azure.com` (keyless, Search MI) |
...
#### (B) `workIQ` KS → **인증 체인 완전 통과**, 최종 **"AI credits" 게이트**에서만 차단
...
- **⭐ 최종 게이트 = Copilot Credits (AI credits):** 시도 3에서 workIQ task가 **3.8초** 소요하며 **실제 Work IQ A2A 에이전트 호출에 도달**했고, 인증이 아니라 **엔타이틀먼트**에서 거부됨:
  > `Could not call WorkIQ A2A agent. AI credits access is not configured for this user. Please request access from your admin.`
  - **2026-06-16 GA 정책:** **커스텀·서드파티 에이전트(= Foundry IQ)가 Work IQ API(A2A)로 호출** 시 **항상 Copilot Credits(사용량 과금) 소비** → 관리자가 **M365 Admin Center에서 consumption billing(spend policy·per-user/group/agent) 활성화** 필요. (query형=변동 크레딧, action/tool=0.1 크레딧/건.)
  - **네이티브 M365 Copilot(대화형 `ask`)은 Copilot 좌석 구독에 포함** → **크레딧 불필요** → **§5.4 `ask`가 200 성공한 이유가 이것으로 확정**.

#### (C) 라이선스 판정 확정 (사용자 최종 목표)

| M365 접근 경로 | 라이선스 / 과금 | 이번 라이브 결과 |
|---|---|---|
| **대화형 `ask`** (native Work IQ, bizchat) | **Copilot 좌석 구독에 포함** (크레딧 불필요) | **§5.4 → 200 + 인용** ✅ |
| **Foundry IQ `workIQ` KS** (Work IQ API / **A2A**) | **Copilot Credits 소비 필수**(2026-06-16 GA) + 피처플래그 승인 + Entra앱/FIC + **동일 테넌트** | 인증 **완전 통과**, credits 게이트만 남음 |
| **Foundry IQ `remoteSharePoint` KS** (Retrieval API) | Copilot 좌석 **또는** Retrieval PAYG($0.10/쿼리) | **200 실데이터** ✅ |
| Graph `/me/*` (원천) | M365 기본(그래프 권한) | §2 → 200 |

> **핵심 통찰:** "Foundry IQ가 Work IQ를 쓴다"는 곧 **Work IQ *API*(A2A)** 를 쓴다는 뜻이며, 이는 **좌석 라이선스와 별개로 Copilot Credits 소비 대상**이다. 반면 사용자가 앱에서 직접 하는 `ask`는 좌석에 포함이다. 이 과금 경계가 **대화형 Work IQ vs 프로그래밍적 Work IQ(Foundry IQ 포함)** 의 가장 실무적인 차이다.

#### (D) 잔여 테스트 리소스 & 완결/정리 방법

라이브 검증으로 생성한 리소스(테넌트 `<TENANT_M365_ID>`): KS `zz-rsp-ks`·`zz-workiq-ks`, KB `zz-rsp-kb`·`zz-workiq-kb`, Entra 앱 `<ENTRA_APP_ID>-…`(+FIC). `workIQ` E2E는 **AI credits만 켜면 즉시 완결**된다.
...
# ▶ 완결(관리자가 M365 Admin Center에서 AI credits/consumption billing 활성화 후):
...
2. **데이터플레인 E2E 실증 완료(실 테넌트 `<TENANT_M365_ID>`).** `remoteSharePoint` KS는 **HTTP 200 + 실제 SharePoint 문서**(Fabrikam/Contoso) 반환; `workIQ` KS는 **FIC→OBO→A2A 인증 체인 완전 통과**(3.8s A2A 도달), 유일한 잔여 게이트는 **Copilot Credits(관리자 billing 활성화)**. 컨트롤플레인(§11.2~11.9)에 더해 데이터플레인까지 검증됨.
3. **데이터플레인 등가성 확정.** `remoteSharePoint`↔§3(Retrieval), `workIQ`↔§5(Work IQ 플랫폼)는 본 보고서가 라이브로 검증한 백엔드와 동일 — 이번에 **동일 테넌트에서 실데이터로 재확인**.
4. **라이선스 경계 확정.** 대화형 `ask`=Copilot 좌석 포함(§5.4 200); **Foundry IQ의 `workIQ` KS=Work IQ API(A2A)=Copilot Credits 소비 필수**(2026-06-16 GA) + 피처플래그 승인 + Entra앱(FIC) + 동일 테넌트. 실 테넌트는 플래그가 **`Registered`(승인 완료)** 였고, 남은 것은 **AI credits(consumption billing) 활성화**뿐 — 관리자가 M365 Admin Center에서 켜면 즉시 E2E 완결.
...
| `…how-to-work-iq.md` | **2026-06-02** | workIQ **최소 payload @2026-05-01**, 단일헤더 OBO, "Copilot 라이선스 필수·비용 발생 가능" |
...
| 8 | **Copilot Credits(AI credits)** 소비 게이트 | ◑ **부분 확증** | how-to "Copilot 라이선스 필수"+"비용 발생"; 정확한 크레딧 게이트는 검색문서 밖(web 확증, 2026-06-16 GA) | 중~높음 |
...
| 항목 | 값 |
|---|---|
| Work IQ 리소스 앱 | `fdcc1f02-fc51-4226-8753-f668596af7f7` (SP objId `<SP_OBJ_WORKIQ>-...`) |
| **Work IQ Tools** 앱(공식명; 라이브 AAD displayName은 `Agent Tools`) | `ea9ffc3e-8a23-4a7d-836d-234d7c7565c1` (SP objId `<SP_OBJ_AGENTTOOLS>-...`) |
...
| **Foundry IQ** Search 서비스(라이브) | `<SEARCH_SERVICE_2>` (eastus, <RESOURCE_GROUP_2>, basic) |
...
| **⭐ 실 E2E Search 서비스** | `<SEARCH_SERVICE>` (westus2, <RESOURCE_GROUP>, basic, **SAMI** principalId `<SEARCH_MI_PRINCIPAL>`) |
| **⭐ workIQ OBO Entra 앱**(생성) | `<ENTRA_APP_ID>` (identifierUri `api://<ENTRA_APP_ID>-…`, scope `access_as_user`, FIC `<FIC_ID>-…`) |
...
| **403 (scope 언급 없음)** | usage-based billing plan 미할당 | Copilot Studio plan 할당 후 15–30분 |
...
| **502 `Could not call WorkIQ A2A agent. AI credits access is not configured for this user`** | 인증은 통과, **Copilot Credits 미활성**(A2A 소비 게이트) | 관리자가 **M365 Admin Center에서 consumption billing 활성화**(2026-06-16 GA) (§11.10-B) |
